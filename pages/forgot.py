...
from utils import init_authenticator, dump_config, send_email
...
with st.expander("Forgot password"):
    ...
    with st.form(key="form_forgot_password"):
        ...
        if submitted:
            ...
            if user:
                ...
                user_id = user.id
                previous_code = db_session.query(VerificationCode).filter(VerificationCode.user_id == user_id).first()
                if previous_code:
                    if previous_code.created_at > datetime.now() - timedelta(minutes=5):
                        st.warning('A code has been already sent to your email. Please check your email.')
                    else:
                    ...
                    random_verification_code = ''.join(random.choices(string.ascii_letters + string.digits, k=6)).replace(' ','') 
                    ...
                    st.success('A link to generate a new password has been sent to your email. Please check your email.')
    ...
    with st.form(key="form_reset_password"):
        ...
        submitted = st.form_submit_button("Submit")
        if submitted:
            verification_code = db_session.query(VerificationCode).filter(VerificationCode.code == code).first()
            if verification_code:
                if verification_code.created_at > datetime.now() - timedelta(minutes=5):
                    st.success('Code is valid')
                    # get the user
                    user = db_session.query(User).filter(User.id == verification_code.user_id).first()
                    # reset the password
                    st.success(user.username)
                    new_password = ''.join(random.choices(string.ascii_letters + string.digits, k=16)).replace(' ','')
                    config['credentials']['usernames'][user.username]['password'] = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8') 
                    # send email
                    ...
                    dump_config(config)
                    st.success('Password has been sent to your email. Please check your email.')
